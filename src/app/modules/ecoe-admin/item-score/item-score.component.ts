...
@Component({
  ...
  styleUrls: ['./item-score.component.less']
})
export class ItemScoreComponent implements OnInit {
        ...
        this.ecoe.itemscore().then( (response:itemEval[]) => {
          ...
          response.forEach((pregunta, index) => {
            response[index] = {
              ...
              if(filter.length == 0)
                filter.push(obj)
              if(filter.every(function (currentElement, index, array){
                return ItemScoreComponent.isFilterEqual(currentElement,obj);
              }))
                filter.push(obj)
          })
          this.filter = filter;
  ...
  filterStationName = (listIndexNames: string[], item: itemEval) => listIndexNames.some(stationName => item.stationName.indexOf(stationName) !== -1);
